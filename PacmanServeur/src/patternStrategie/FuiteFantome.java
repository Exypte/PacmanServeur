...
public class FuiteFantome implements Strategies {
	...
	public AgentAction getAction(Agent agent, Maze labyrinth) {
		...
		//calcul distance entre agent et pacman(s)
		...
		//on prend la distance la plus longue
		...
		PositionAgent positionLaPlusDistante=postitionPossibles.get(0);
		
		double maximum =listeDistances.get(0);
		
		for (int i = 0; i < listeDistances.size(); i++) {
			if(maximum < listeDistances.get(i)) {
				maximum=listeDistances.get(i);
				positionLaPlusDistante=postitionPossibles.get(i);
			}
		}
		
		
		return new AgentAction(positionLaPlusDistante.getDir());
	}
}
